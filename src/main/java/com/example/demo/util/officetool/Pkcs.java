...
public class Pkcs {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String issuerStr = "CN=法信,OU=法证中心,O=法信公证云（厦门）科技有限公司,C=CN,E=gzykf@qq.com,L=厦门,ST=厦门";
        String subjectStr = "CN=法信,OU=法证中心,O=法信公证云（厦门）科技有限公司,C=CN,E=gzykf@qq.com,L=厦门,ST=厦门";
        String certificateCRL = "https://www.egongzheng.com";
        Map<String, byte[]> result = createCert("www.egongzheng.com", issuerStr, subjectStr, certificateCRL);
        // ca.jks
       /* FileOutputStream outPutStream = new FileOutputStream("E:\\home/keystore.p12");
        outPutStream.write(result.get("keyStoreData"));
        outPutStream.close();
        FileOutputStream fos = new FileOutputStream(new File("E:\\home/keystore.cer"));
        ...
        InputStream keyStoreStream = new ByteArrayInputStream(result.get("keyStoreData"));
        InputStream keyStoreStreamNew = new ByteArrayInputStream(result.get("keyStoreData"));
        String keyStorePassword = "www.egongzheng.com";
        String reason = "电子签名";
        String location = "厦门";
        ...
        // 文本签章
        Image signImageText = Image.getInstance(SignPdf.getImage(chapterContent).toByteArray());

        // 首次签订
        ByteArrayOutputStream byteOut = SignPdf.sign(in, keyStoreStream, signImage, "当事人签署：", reason, location);
        in.close();
        if (byteOut == null) {
            return;
        }
        // 多次签订
        byteOut = SignPdf.sign(new ByteArrayInputStream(byteOut.toByteArray()), keyStoreStreamNew,
                signImageText, "厦门市鹭江公证处", reason, location);
        if (byteOut == null) {
            return;
        }
        try (OutputStream out = new FileOutputStream("E:\\home\\ff.pdf")) {
